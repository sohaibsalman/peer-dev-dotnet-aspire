...
public class QuestionsController(QuestionDbContext db) : ControllerBase
{
    [Authorize]
    [HttpPost]
    public async Task<ActionResult<Question>> CreateQuestion(CreateOrUpdateQuestionDto dto)
    {
        var missing = await ValidateTags(dto);
    ...
    [Authorize]
    [HttpPut("{id}")]
    public async Task<ActionResult> UpdateQuestion(string id, CreateOrUpdateQuestionDto dto)
    {
        var question = await db.Questions.FindAsync(id);
    ...
    private async Task<List<string>> ValidateTags(CreateOrUpdateQuestionDto dto)
    {
